...
Possono essere mappati a valori booleani (true/false). Collegabile da un capo a GND e dall'altro capo a un GPIO del microcontrollore in modalità
input pullup. Può essere utilizzato nel codice mediante la funzione digitalRead o come switch component su esphome. Esempio di collegamento
...
\begin{figure}[h]
  ...
  \begin{circuitikz} \draw
    (0,0) -- (0,4)
     -- (4,4)
    to[twoport,t={ESP}] (4,0)
    -- (0,0)
  ;
  \draw (0,4) -- ++(1,0) node[rotary switch, anchor=in](){}; 
  \end{circuitikz}
...
integrato nel microcontrollore. Per poter leggere l'output è necessario collegarlo a un GPIO che supporta la lettura di segnali analogici
e nel codice utilizzare la funzione analogRead. Su esphome è necessario usare un sensore di tipo adc.
