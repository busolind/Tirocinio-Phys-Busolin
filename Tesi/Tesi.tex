...
\todo{busolin: meglio metterlo negli interventi?}
...
ma prima una parentesi storica su alcuni tipi di strumenti utilizzabili come "visualizzatori" e su come funziona uno strumento ad ago
(più frequente oggetto di visualizzazione, nonché quello usato nell'esperimento di conversione nell'ambito del tirocinio).

\subsection{Panoramica storica sugli strumenti di "visualizzazione"}
\paragraph{Dekatron}
...
relegata all'utilizzo hobbystico, ad esempio per la realizzazione di particolari orologi. Figura~\ref{fig:dekatron}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\textwidth]{dekatron}
  \caption{Dekatron}
  \label{fig:dekatron}
\end{figure}

\paragraph{Occhio magico}
Valvola termoionica usata per rappresentare l'intensità di un segnale elettrico. Storicamente impiegata nelle radio come
indicatore di sintonia. Figura~\ref{fig:occhiomagicofasi}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\textwidth]{occhiomagicofasi}
  \caption{Occhio magico in diverse fasi di illuminazione}
  \label{fig:occhiomagicofasi}
\end{figure}

\paragraph{Oscilloscopio}
Strumento utilizzato per visualizzare l'andamento nel tempo di un segnale in maniera grafica. Se pilotato a dovere è in grado di
...
\chapter{Realizzazione del prototipo - IoTDial}
...
e alcune osservazioni al riguardo.
...
impostare un intervallo di tempo tra esecuzioni ripetute di una procedura. \todo{busolin: Integrare tutto come task}

\subsection{Configurazione via MQTT}
Come modalità di modifica ``a runtime" della configurazione, è stata implementata la possibilità di inviare messaggi MQTT al topic
\texttt{Phys/setFromJSON} per impostare l'intera configurazione, altrimenti i campi singoli possono essere settati tramite i topic:
\begin{itemize}
...
Come descritto in precedenza, la configurazione poteva essere alterata anche tramite una pagina web accessibile all'indirizzo IP
nella rete locale del microcontrollore o il suo hostname.
